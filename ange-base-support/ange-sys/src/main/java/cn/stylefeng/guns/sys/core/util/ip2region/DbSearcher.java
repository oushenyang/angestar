...
public class DbSearcher 
{
    ...
    */
    ...
    public DbSearcher(DbConfig dbConfig, byte[] dbBinStr)
    {
        ...
        firstIndexPtr = IpToRegionUtil.getIntLong(dbBinStr, 0);
        lastIndexPtr  = IpToRegionUtil.getIntLong(dbBinStr, 4);
    ...
    */
    public DataBlock memorySearch(long ip) throws IOException
    {
        ...
        while ( l <= h ) {
            ...
            sip = IpToRegionUtil.getIntLong(dbBinStr, p);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IpToRegionUtil.getIntLong(dbBinStr, p + 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IpToRegionUtil.getIntLong(dbBinStr, p + 8);
                    break;
                }
            }
        }
        ...
        int city_id = (int) IpToRegionUtil.getIntLong(dbBinStr, dataPtr);
    ...
    */
    ...
    {
        return memorySearch(IpToRegionUtil.ip2long(ip));
    }
    ...
    */
    public DataBlock getByIndexPtr( long ptr ) throws IOException
    {
        ...
        long extra = IpToRegionUtil.getIntLong(buffer, 8);
        ...
        int city_id = (int) IpToRegionUtil.getIntLong(data, 0);
    ...
    public DataBlock btreeSearch( long ip ) throws IOException
    {
        ...
        if ( HeaderSip == null )  {
            ...
            for ( int i = 0; i < b.length; i += 8 ) {
                startIp = IpToRegionUtil.getIntLong(b, i);
                dataPtr = IpToRegionUtil.getIntLong(b, i + 4);
                if ( dataPtr == 0 ) break;
        ...
        while ( l <= h ) {
            int m = (l + h) >> 1;
            int p = m * blen;
            sip = IpToRegionUtil.getIntLong(iBuffer, p);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IpToRegionUtil.getIntLong(iBuffer, p + 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IpToRegionUtil.getIntLong(iBuffer, p + 8);
                    break;
                }
            }
        }
    ...
    */
    ...
    {
        return btreeSearch(IpToRegionUtil.ip2long(ip));
    }
    ...
    */
    public DataBlock binarySearch( long ip ) throws IOException
    {
        ...
        if ( totalIndexBlocks == 0 ) {
            ...
            //initialize the global vars
            firstIndexPtr = IpToRegionUtil.getIntLong(superBytes, 0);
            lastIndexPtr = IpToRegionUtil.getIntLong(superBytes, 4);
            totalIndexBlocks = (int)((lastIndexPtr - firstIndexPtr)/blen) + 1;
        }
        ...
        while ( l <= h ) {
            ...
            raf.readFully(buffer, 0, buffer.length);
            sip = IpToRegionUtil.getIntLong(buffer, 0);
            if ( ip < sip ) {
                h = m - 1;
            } else {
                eip = IpToRegionUtil.getIntLong(buffer, 4);
                if ( ip > eip ) {
                    l = m + 1;
                } else {
                    dataptr = IpToRegionUtil.getIntLong(buffer, 8);
                    break;
                }
            }
        }
    ...
    */
    ...
    {
        return binarySearch(IpToRegionUtil.ip2long(ip));
    }
