...
import cn.stylefeng.guns.sys.core.util.DESCBCUtil;
 ...
 */
@Controller
@RequestMapping("web/api/v4")
public class NewTowHuanYingV4Controller {
    ...
    public String appurl(){
        ...
        if ("E19156B0683CD93E".equals(pkgId)){
            AppUrlResult appUrlResult = new AppUrlResult("e19156b0683cd93e","https://dldir1.qq.com/weixin/android/weixin806android1900.apk",1);
            String aaa = DESCBCUtil.encode(JSON.toJSONString(appUrlResult));
            assert aaa != null;
...
//            }
        ...
        if (appversioncode.equals("210")){
            isShow = appPowerService.whetherLegalBySignAndAppCodeNoInsert(sign,applicationName,virtualId,"huanyin210");
        }else {
            return null;
        }
        ...
        if ("com.alibaba.android.rimet".equals(app_pkg)){
            ...
            if (appversioncode.equals("210")){
                dicts = dictService.listDictsByCodeByRedis("HUANYING_TOKEN210");
            }else {
            ...
            for (Dict dict : dicts){
                if (dict.getName().equals(app_version)){
                    if (appversioncode.equals("210")){
                        ...
                        String aaa = DESCBCUtil.encode(JSON.toJSONString(appTokenResult));
                        assert aaa != null;
                        aaa = aaa.replaceAll("\r|\n", "");
                        map.put("data", aaa);
                    }else {
