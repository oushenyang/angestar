 ...
 */
@Controller
@RequestMapping("/api/shenqi")
public class DingWeiShenQiController {
    ...
    public JSONObject AppBlackList(@RequestBody String  body){
        String av = HttpContext.getRequest().getParameter("av");
        String appName = HttpContext.getRequest().getParameter("n");
        ...
        String token = HttpContext.getRequest().getParameter("bundleid");
        String virtualId = HttpContext.getRequest().getParameter("c");
        String application = HttpContext.getRequest().getParameter("sng");
        String sign;
...
//        boolean whetherLegal = false;
        if (StringUtils.isNotEmpty(pkg)&&StringUtils.isNotEmpty(av)){
            if (pkg.equals("com.alibaba.android.rimet")&&av.equals("1.2.20")){
...
//                        }
                ...
                if (!a.contains("latitude")){
                    aps.setLongitude(null);
                    aps.setLatitude(null);
                    Map map = new HashMap<String, String>();
                    map.put("longitude","");
                    map.put("latitude", "");
                    map.put("t", aps.getT());
                    map.put("w", aps.getW());
                    map.put("t1", aps.getT1());
                    map.put("w1", aps.getW1());
                    aaa = CreateNamePicture.encrypt(JSON.toJSONString(map), Charset.forName("utf8"), "00122897");
                }else {
